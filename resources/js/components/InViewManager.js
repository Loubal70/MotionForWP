...
export class MotionTarget {
	...
	getEasing() {
		...
		if ( ! motionForWPEasings[ easingSlug ] )
			return motionForWPOptions.easing ?? 'ease-in-out';
		return motionForWPEasings[ easingSlug ].property;
	}

	getResetScroll() {
		return (
			this.element.getAttribute( 'data-motion-resetScroll' ) ??
			motionForWPOptions.resetScroll ??
			false
		);
	}
}
